...
If filtering based on custom fields containing dots is required, they can be specified using a json formatted where clause based on the formatting of the Civi API query, replacing `[` with `(` due to WordPress shortcode limitations. For example: `where='(("field1","=","value"),("field2","=","value"))'`.
